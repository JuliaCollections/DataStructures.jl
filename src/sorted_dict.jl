...
SortedDict(o1::Ordering, o2::Ordering) =
    throw(ArgumentError("SortedDict with two parameters must be called with an Ordering and an interable of pairs"))
...
function mergetwo!(m::SortedDict{K,D,Ord}, m2::Associative{K,D}) where {K,D,Ord<:Ordering}
    for (k,v) in m2
...
function merge!(m::SortedDict{K,D,Ord}, others::Associative{K,D}...) where {K,D,Ord<:Ordering}
    for o in others
        mergetwo!(m,o)
    end
end
