...
function _heap_bubble_up!(comp::Comp, nodes::Vector{MutableBinaryHeapNode{T}},
                          nodemap::Vector{Int}, nd_id::Int) where {Comp,T}
...
function _heap_bubble_down!(comp::Comp, nodes::Vector{MutableBinaryHeapNode{T}},
                            nodemap::Vector{Int}, nd_id::Int) where {Comp,T}
    @inbounds nd = nodes[nd_id]
    v::T = nd.value
...
function _binary_heap_pop!(comp::Comp, nodes::Vector{MutableBinaryHeapNode{T}},
                           nodemap::Vector{Int}) where {Comp,T}
    # extract root node
    rt = nodes[1]
    v = rt.value
...
pop!(h::MutableBinaryHeap) = _binary_heap_pop!(h.comparer, h.nodes, h.node_map)

"""
    update!(h::MutableBinaryHeap{T}, i::Int, v::T) where T
