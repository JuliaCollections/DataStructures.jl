...
Trie(kv::AbstractVector{Tuple{K,V}}) where {K,V} = Trie{V}(kv)
Trie(kv::Associative{K,V}) where {K,V} = Trie{V}(kv)
Trie(ks::AbstractVector) = Trie{Void}(ks, similar(ks, Void))
...
iteratorsize(::Type{TrieIterator}) = Base.SizeUnknown()
