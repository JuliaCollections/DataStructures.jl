...
A classified collection is a mapping that associates a collection to each key. The collection
can be any data structure that supports `Base.push!`, e.g. `Array` and `Set`.
"""
...
pop!(cc::ClassifiedCollections{K,C}, key::K) where {K,C} = pop!(cc.map, key)
